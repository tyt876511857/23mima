...
class IndexAction extends Action{
	...
	//通过手机验证获取报告列表
	function getBaogaoList() {
		if (empty($_SESSION['phone']) || empty($_SESSION['yzm'])) {//此处是第yi次登入
			if (empty($_SESSION['index_userid'])) {//openid用户，没记录直接让他去注册
				$url = '/index/Index/queryBaogao';
		...
		//这里做没有报告的查询报告，新增用户
		if($_POST['phone']==$_SESSION['phone'] && $_POST['yzm']==$_SESSION['yzm']){//第一次查询
