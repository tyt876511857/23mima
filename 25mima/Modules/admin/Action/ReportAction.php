...
class ReportAction extends CommonAction{
		...
		if (!empty($userInfos)) {
			...
			foreach ($this->list as &$lvalue) {
				if (isset($userArr[$lvalue['uid']])) {
					$lvalue['token'] = $userArr[$lvalue['uid']]['token'];
				}
			}
		}
		$this->display();
	...
	//手动通知第三方
	public function baogao_notifythird() {
		//通知同程报告已产生
		$id = intval($_GET['id']);
		$uid = intval($_GET['uid']);
		$sql = 'select * from gs_user where id='.$uid;
		$userInfo = $this->db->getRow($sql);
		if (empty($userInfo['thirdtoid'])) {
			return false;
		}

		$param['url'] = 'http://m.ly.com/itravel/Home/OrderStatus';
		$param['date'] = date('Y-m-d H:i',$userInfo['tokentime']);
		$param['token'] = $userInfo['token'];
		$param['supplier'] = '23mima';
		$param['status'] = 15;
		$param['userid'] = $userInfo['thirduid'];
		$param['productId'] = $userInfo['thirdtoid'];
		$info = 'http://23mima.com/index/third/baogao/id/'.$id.'?k='.md5(C('THIRD_KEY').$id);
		$param['info'] = urlencode($info);
		$param['sign'] = md5(C('THIRD_KEY').$param['status'].$param['userid'].$param['productId']);
		...
		if ($notifyRes->IsSuccess != 'true') {
			...
			//增加通知用户报告已出
			$zhuangtai = '检测状态:您的采样盒已送至实验室，请于25个工作日内臻旅个人中心查看电子检测报告。';
			$phone = $userInfo['username'];
			file_get_contents(__ROOT__ ."/data/duanxin/SendTemplateSMS.php?phone=$phone&zhuangtai=$zhuangtai");
			$psql = 'update gs_user set token=\'\' where id='.$uid;
