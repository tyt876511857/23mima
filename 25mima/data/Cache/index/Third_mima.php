...
.web_toast{
    ...
    border-radius: 4px;
		...
		<script>
			$(function() {
				...
				var pNum;
				var $baseWidth = $(window).width()/2-400;
				$("#footer").css('width','800px')
				$("#footer").css('left',$baseWidth+'px')

			});
		</script>
	</head>

	<body style="background: #F1F3F2;height: 880px;width: 800px;margin: 0 auto;">
		<section class="ui-container" >
			<form id="formmit" action="<?php echo U('Third/runmima') ?>" method="post" onsubmit='return run();'>
				<ul class="ui-row ui-whitespace buy_car">
					...
					<li class="ui-row-flex">
						...
						<div class="ui-col ui-col-2 b_c_detial">
							...
							<p class="b_c_d_price" style="text-decoration: line-through;color:#736969;">￥<b><?php if(isset($v['shop_price'])){echo $v['shop_price'];} ?></b></p>
						</div>
					</li>
					<li class="ui-row-flex">
						<div class="ui-col ui-col-2"><span>激活数量：</span></div>
						<div class="ui-col ui-col-2 but_num">
							<span><a class="del">-</a><input disabled="disabled" type="text" name="dnumber[]" value="<?php if(isset($v['number'])){echo $v['number'];} ?>" class="product_num"/><input  type="hidden" name="number[]" value="<?php if(isset($v['number'])){echo $v['number'];} ?>" class="product_num"/><a  class="add">+</a></span>
						</div>
						<script>
				...
				<div class="ui-row ui-whitespace">
					<div class="ui-form ui-border-t">
						...
						<div class="ui-form-item ui-form-item-textarea ui-border-b">
							<label>详细地址：</label>
							<textarea onfocus="textfocus(this)" onblur="textblur(this)" placeholder="(注:邮寄选择到付)" name="address"></textarea>
						</div>
				...
				<div id="footer" class="ui-footer ui-footer-stable ui-btn-group ui-border-t ui-row-flex ">
 ...
 /*  function textfocus(o){
   		$("#formmit").css('position','fixed');
   		$("#formmit").css('bottom','50px');
    }
    function textblur(o){
    	$("#formmit").css('position','static');
   		$("#formmit").css('bottom','auto');
    }*/
  function dopost() {
  	var ajaxCallUrl = '<?php echo U('Third/runmima') ?>';
  	$res = run();
  	if (!$res) {
  		return false;
  	}
...
var ret = "<div class='web_toast'><div class='cx_mask_transparent'></div>" + dcfg.msg + "</div>";
  ...
  function showalert(url) {
  	var geneUrl = '<?php echo C('THIRD_URL')?>';
  	if (url == undefined || url != geneUrl) {
  		alert('接口通知权益扣减失败');
  		console.log(url);
  		var url = geneUrl;
  	}
