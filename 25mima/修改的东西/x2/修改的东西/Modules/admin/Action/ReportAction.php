...
class ReportAction extends CommonAction{
	...
	public function doUpdateTeChang($updateArr=array(),$cid=0,$fitArr) {
		...
		if (!empty($updateArr)) {
			...
			$sql .= $commonSet.' END, '. $goodSet.' END, '.$excellentSet;
	...
	function baogao(){
		...
		$this->data = $data;
		$sql = 'select b.*,t.id as tid,t.name,t.time5  from '.$this->baogao.' b left join '.$this->tuoye.' as t on t.identifier=b.title '.$where.' order by id desc  limit '.$page*$pagesize.','.$pagesize;
		$this->list = $this->db->getAll($sql);
		$this->display();
	}
	function baogao_add(){
