						...
						if ($j == 'rs1229984') {
							switch ($field['contentCT'][$j][1]) {
								case 'GG':
									...
									$field['contentCT'][$j][4]='<span style="color:#8dc21f;">弱</span>';
									break;
								case 'AG':
									$field['contentCT'][$j][3]='乙醇代谢较快';
									$field['contentCT'][$j][4]='<span style="color:#f39800;">较弱</span>';
									break;
								case 'AA':
									$field['contentCT'][$j][3]='乙醇代谢快';
									$field['contentCT'][$j][4]='<span style="color:#c30d23;">正常</span>';
									break;
								case 'GA':
									$field['contentCT'][$j][3]='乙醇代谢较快';
									$field['contentCT'][$j][4]='<span style="color:#f39800;">较弱</span>';
...
';
			...
			$zheye = $this->buildCT($tilestr,$field['cid'],$zheye);
			//krsort($new_zheye);
			...
			if ($field['cid']==5) {
				...
				if ($this->fzheye['0']['fenshu1'] == 5) {
					$this->fenxi = "运动能力强，运动量大的话，建议多补充营养物质，合理安排运动量。";
				} elseif ($this->fzheye['0']['fenshu1'] == 4) {
					$this->fenxi = "运动能力良好，建议多参加体育运动，注意运动前做好准备活动，选择合适运动量，防止运动时受伤。";
				} else {
					$this->fenxi = "运动能力一般，建议多参加体育运动，生命在于运动，选择合适的运动项目，提高免疫力和身体素质。";
				}
	...
	function runmima(){
		...
		$data['size'] = $_POST['province'].$_POST['city'].$_POST['country'].$_POST['address'].' 年龄：'.intval($_POST['old']);
	...
	//通用组成用户检测的表格数据
	function buildCT($tilestr,$cid,$zheye) {
		if (!empty($tilestr)) {
			$ctsql = 'select p.id,p.cid,p.title,p.title_c,p.name,f.name as fname,p.description,p.SNP,p.chr,p.litpic,p.shuoming,f.secdes from gs_bg_property p left join gs_bg_fenshu f on p.id=f.pid where title in('.substr($tilestr, 0, -1).') and cid='.$cid;
			$ctlist = $this->db->getAll($ctsql);
			if (!empty($ctlist)) {
				foreach ($ctlist as $value) {
					$temp[$value['title']] = $value;
					$temp[$value['title'].$value['fname']] = $value['secdes'];
				}
