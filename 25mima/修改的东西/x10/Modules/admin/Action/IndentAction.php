...
Class IndentAction extends CommonAction{
	...
	public function update(){
		...
		//支付宝提交发货信息
		$sql = 'select WIDtrade_no,type,i_type,phone,name from '.$this->table.' where id='.$_POST['id'];
		...
		if($r){
			//同城的物流通知
			if ($list['i_type']==11) {
				$zhuangtai = '尊敬的'.$list['name'].'，您的Vtravel臻享卡基因检测设备已寄出，'.$_POST['company'].'：'.$_POST['express'].',请注意查收!';
				$phone = $list['phone'];
				file_get_contents(__ROOT__ ."/data/duanxin/SendTemplateSMS.php?phone=$phone&tzhuangtai=".$list['name']."&company=".$_POST['company']."&express=".$_POST['express']);
			}
			$this->success('提交成功！');
		}
	}
